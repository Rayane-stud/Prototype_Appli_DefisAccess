...
IA_CONF_MIN_MIXTE = 0.27  # seuil de confiance IA en dessous duquel une détection est ignorée dans la fusion mixte
...
def main(rdv_lat: float, rdv_long: float, nb_equipes: int, ville: str):
    ...
    # ── Détection des passages piétons : IA (YOLO, filtrée par confiance et
    # par distance géométrique) fusionnée avec OSM+Accidents ───────────
    ...
    tab_croisement = fusion_mixte.calculer_mixte(
        tab_croisement, ville, BASE_DIR,
        conf_min=IA_CONF_MIN_MIXTE, dossier_images=dossier_images,
    )
    ...
    if rep=="o":
        ...
        nom="data/raw/comparaisons/"+ville+"mixte.csv"
