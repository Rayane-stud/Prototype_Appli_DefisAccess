...
def filtre_Distance (df_lieux, df_croisement, points: list, rayon_km: float = 0.2):
    ...
    for v in points:
        ...
        #on identifie les intersections a moins de 200 mètres des points majeurs

        df.loc[df[f"dist_{v}"] < rayon_km, "pres_PM"] = 1

       

        #supprime les colonnes temporaire crées pour la comparaison de distance
...
def fusion_croisement(df, threshold_km: float = 0.03):
    ...
    df["Intersections"] = df.apply(
        ...
        lambda row: row["Intersections"] + "/" + row["intersection_n_1"]

        if pd.notna(row["dist_inter"]) and row["dist_inter"] < threshold_km

        else row["Intersections"], axis=1)
...
df=filtre_Distance(df_lieux, df_croisement, Points_Majeur, rayon_km=0.2)
